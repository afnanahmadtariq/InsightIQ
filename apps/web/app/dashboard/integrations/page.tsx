...
const descriptions: Record<string, string> = {
  ...
  'evidence-normalization': 'The worker extracts bounded claims with confidence and direct source relationships.',
  'brief-synthesis': 'Meeting and outreach recommendations are generated from accepted evidence only.',
  'notification-delivery': 'In-app notifications tell the owner when a conversation brief is ready.',
}
...
export default async function Page() {
  ...
  return <WorkspacePage>
    <WorkspaceHeader eyebrow="Data sources" title="Know what powers each brief." lead="Provider status and evidence safeguards for this workspace."/>
    <WorkspaceSplit>
      <section className="rounded-[17px] border border-iq-200 bg-white p-[23px]"><header className="flex items-center justify-between gap-4"><span className="grid size-[42px] place-items-center rounded-xl bg-[linear-gradient(135deg,#dff4ff,#e2ecff)] text-brand"><Search size={20}/></span><StatusBadge status={capabilities.webDiscovery.status}/></header><h2 className="mt-5 mb-[7px] text-[1.08rem] text-iq-900">{capabilities.webDiscovery.provider} web discovery</h2><p className="m-0 text-[.83rem] leading-[1.6] text-iq-600">{capabilities.webDiscovery.status === 'configured' ? 'A dedicated API key is configured for reliable production discovery.' : 'The SDK is active in shared keyless evaluation mode. Add TAVILY_API_KEY to production before sustained usage.'}</p><dl className="mt-5 mb-0 grid grid-cols-2 gap-2.5"><CapabilityStat label="Search depth" value={capabilities.webDiscovery.searchDepth}/><CapabilityStat label="Results per query" value={capabilities.webDiscovery.maxResults}/></dl></section>
