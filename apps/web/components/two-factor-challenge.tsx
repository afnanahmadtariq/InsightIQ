...
export function TwoFactorChallenge({ context }: { context: AccountContext }) {
  ...
  return <form className="grid gap-4" onSubmit={verify}><FormMessage>{sent ? `A code was sent to ${context.user.email}.` : 'Send a one-time code to your verified email.'}</FormMessage>{!sent && <Button className="w-full" type="button" variant="secondary" onClick={send} disabled={pending}>Send email code</Button>}<Field id="code" name="code" label="Six-digit security code" inputMode="numeric" pattern="[0-9]{6}" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} required/>{error && <FormMessage tone="error">{error}</FormMessage>}<Button className="w-full" type="submit" disabled={pending || !sent || code.length !== 6}>{pending ? 'Checking…' : 'Verify identity'}</Button></form>
}
